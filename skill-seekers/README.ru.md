...
> WARNING: **Уведомление о машинном переводе**
...
- PASS: **Проверено в бою** — 2 540+ тестов, 24+ пресетов для фреймворков, готово к продакшену
...
- PASS: **Поддержка llms.txt** — автоматическое обнаружение и использование LLM-ready файлов документации (в 10 раз быстрее)
- PASS: **Универсальный сканер** — работает с ЛЮБЫМ сайтом документации
- PASS: **Умная категоризация** — автоматическая организация контента по темам
- PASS: **Определение языка кода** — распознавание Python, JavaScript, C++, GDScript и других
- PASS: **24+ готовых пресетов** — Godot, React, Vue, Django, FastAPI и другие

### Поддержка PDF
- PASS: **Базовое извлечение PDF** — извлечение текста, кода и изображений из PDF-файлов
- PASS: **OCR для сканированных PDF** — извлечение текста из сканированных документов
- PASS: **PDF с паролем** — обработка зашифрованных PDF
- PASS: **Извлечение таблиц** — извлечение сложных таблиц из PDF
- PASS: **Параллельная обработка** — в 3 раза быстрее для больших PDF
- PASS: **Умное кэширование** — на 50% быстрее при повторных запусках

### Извлечение из видео
- PASS: **YouTube и локальные видео** — извлечение субтитров, кода и структурированных знаний из видео
- PASS: **Анализ визуальных кадров** — OCR-извлечение из редакторов кода, терминалов, слайдов и диаграмм
- PASS: **Автоопределение GPU** — автоматическая установка правильной сборки PyTorch (CUDA/ROCm/MPS/CPU)
- PASS: **ИИ-улучшение** — двухэтапное: очистка артефактов OCR + генерация отполированного SKILL.md
- PASS: **Обрезка по времени** — извлечение определённых фрагментов с `--start-time` и `--end-time`
- PASS: **Поддержка плейлистов** — пакетная обработка всех видео в плейлисте YouTube

### Анализ репозиториев GitHub
- PASS: **Глубокий анализ кода** — AST-разбор для Python, JavaScript, TypeScript, Java, C++, Go
- PASS: **Извлечение API** — функции, классы, методы с параметрами и типами
- PASS: **Метаданные репозитория** — README, дерево файлов, распределение языков, звёзды/форки
- PASS: **GitHub Issues и PR** — получение открытых/закрытых issues с метками и вехами
- PASS: **CHANGELOG и релизы** — автоматическое извлечение истории версий
- PASS: **Обнаружение конфликтов** — сравнение документированных API с фактической реализацией кода
- PASS: **MCP-интеграция** — на естественном языке: «Просканируй GitHub-репозиторий facebook/react»

### Унифицированное мультиисточниковое сканирование
- PASS: **Объединение нескольких источников** — смешивайте документацию + GitHub + PDF в одном навыке
- PASS: **Обнаружение конфликтов** — автоматическое нахождение расхождений между документацией и кодом
- PASS: **Умное слияние** — на основе правил или с помощью ИИ
- PASS: **Прозрачная отчётность** — сравнение бок о бок с предупреждениями WARNING:
- PASS: **Анализ пробелов в документации** — выявление устаревшей документации и недокументированных функций
- PASS: **Единый источник истины** — один навык показывает и намерение (документация), и реальность (код)
- PASS: **Обратная совместимость** — устаревшие одноисточниковые конфигурации продолжают работать

### Поддержка нескольких LLM-платформ
- PASS: **12 LLM-платформ** — Claude AI, Google Gemini, OpenAI ChatGPT, MiniMax AI, универсальный Markdown, OpenCode, Kimi, DeepSeek, Qwen, OpenRouter, Together AI, Fireworks AI
- PASS: **Универсальное сканирование** — одна и та же документация для всех платформ
- PASS: **Платформоспецифичная упаковка** — оптимизированные форматы для каждой LLM
- PASS: **Экспорт одной командой** — флаг `--target` для выбора платформы
- PASS: **Опциональные зависимости** — устанавливайте только то, что нужно
- PASS: **100% обратная совместимость** — существующие рабочие процессы Claude без изменений

| Платформа | Формат | Загрузка | Улучшение | API Key | Пользовательский эндпоинт |
|-----------|--------|----------|-----------|---------|--------------------------|
| **Claude AI** | ZIP + YAML | PASS: Авто | PASS: Да | ANTHROPIC_API_KEY | ANTHROPIC_BASE_URL |
| **Google Gemini** | tar.gz | PASS: Авто | PASS: Да | GOOGLE_API_KEY | - |
| **OpenAI ChatGPT** | ZIP + Vector Store | PASS: Авто | PASS: Да | OPENAI_API_KEY | - |
| **Универсальный Markdown** | ZIP | FAIL: Вручную | FAIL: Нет | - | - |
...
- PASS: **LangChain Documents** — прямой экспорт в формат `Document` с `page_content` + метаданными
...
- PASS: **LlamaIndex TextNodes** — экспорт в формат `TextNode` с уникальными ID + эмбеддингами
...
- PASS: **Формат, готовый к Pinecone** — оптимизирован для загрузки в векторную базу данных
...
- PASS: **Cursor IDE** — генерация `.cursorrules` для ИИ-подсказок при написании кода
...
- PASS: **Windsurf** — настройка контекста ИИ-помощника Windsurf через `.windsurfrules`
...
- PASS: **Cline (VS Code)** — системные промпты + MCP для VS Code-агента
...
- PASS: **Continue.dev** — контекстные серверы для IDE-независимого ИИ
...
- PASS: **Трёхпоточный анализ** — разделение GitHub-репозитория на потоки «Код», «Документация» и «Аналитика»
- PASS: **Унифицированный анализатор кодовой базы** — работает как с URL GitHub, так и с локальными путями
- PASS: **C3.x как глубина анализа** — выбор «basic» (1–2 мин) или «c3x» (20–60 мин)
- PASS: **Расширенная генерация маршрутизатора** — метаданные GitHub, быстрый старт из README, типичные проблемы
- PASS: **Интеграция Issues** — распространённые проблемы и решения из GitHub Issues
- PASS: **Умные ключевые слова маршрутизации** — метки GitHub с двойным весом для лучшего определения тем
...
- PASS: **Система конфигурации с несколькими токенами** — управление несколькими аккаунтами GitHub (личный, рабочий, open source)
...
- PASS: **Интерактивный мастер настройки** — красивый терминальный интерфейс для простой настройки
- PASS: **Умный обработчик лимитов запросов** — больше никаких бесконечных ожиданий!
...
- PASS: **Возобновление** — продолжение прерванных задач
- PASS: **Поддержка CI/CD** — флаг `--non-interactive` для автоматизации
...
- PASS: **Git-источники конфигураций** — получение конфигураций из приватных/командных Git-репозиториев
- PASS: **Управление несколькими источниками** — регистрация неограниченного количества репозиториев GitHub, GitLab, Bitbucket
- PASS: **Командная работа** — обмен пользовательскими конфигурациями в командах из 3–5 человек
- PASS: **Корпоративная поддержка** — масштабирование до 500+ разработчиков
- PASS: **Безопасная аутентификация** — токены через переменные окружения (GITHUB_TOKEN, GITLAB_TOKEN)

### Анализ кодовой базы (C3.x)

**C3.4: Извлечение паттернов конфигурации с ИИ-улучшением**
- PASS: **9 форматов конфигурации** — JSON, YAML, TOML, ENV, INI, Python, JavaScript, Dockerfile, Docker Compose
- PASS: **7 типов паттернов** — база данных, API, логирование, кэш, почта, аутентификация, сервер
- PASS: **ИИ-улучшение** — опциональный двухрежимный ИИ-анализ (API + LOCAL)
- PASS: **Анализ безопасности** — обнаружение жёстко закодированных секретов и открытых учётных данных

**C3.3: ИИ-улучшенные пошаговые руководства**
- PASS: **Полное ИИ-улучшение** — преобразование базовых руководств в профессиональные учебники
- PASS: **5 автоматических улучшений** — описание шагов, устранение неполадок, предварительные требования, следующие шаги, сценарии использования
- PASS: **Двухрежимная поддержка** — API-режим (Claude API) или LOCAL-режим (Claude Code CLI)
- PASS: **Нулевые затраты в LOCAL-режиме** — БЕСПЛАТНОЕ улучшение с вашим планом Claude Code Max
...
- PASS: **5 встроенных пресетов** — `default`, `minimal`, `security-focus`, `architecture-comprehensive`, `api-documentation`
- PASS: **Пользовательские пресеты** — добавляйте собственные рабочие процессы в `~/.config/skill-seekers/workflows/`
- PASS: **Цепочки рабочих процессов** — объединяйте два или более рабочих процесса в одной команде
- PASS: **Полное управление через CLI** — просмотр, копирование, добавление, удаление и валидация рабочих процессов
...
- PASS: **Асинхронный режим** — сканирование в 2–3 раза быстрее с async/await (флаг `--async`)
- PASS: **Поддержка большой документации** — обработка документов на 10K–40K+ страниц с умным разделением
- PASS: **Маршрутизатор/Hub-навыки** — интеллектуальная маршрутизация к специализированным поднавыкам
- PASS: **Параллельное сканирование** — одновременная обработка нескольких навыков
- PASS: **Контрольные точки/Возобновление** — прогресс никогда не теряется при длительном сканировании
- PASS: **Система кэширования** — сканируйте один раз, пересобирайте мгновенно

### PASS: Контроль качества
- PASS: **Полное покрытие тестами** — 2 540+ тестов с обширным покрытием
...
```
 ...
 ФАЗА 4: Упаковка навыка
  ФАЗА 5: Загрузка в Claude (опционально, требуется API Key)
```
...
| Загрузка | PASS: API | PASS: API | PASS: API | FAIL: Вручную |
| Улучшение | PASS: Sonnet 4 | PASS: 2.0 Flash | PASS: GPT-4o | FAIL: Нет |
| Все режимы навыков | PASS: | PASS: | PASS: | PASS: |
...
- WARNING: **Несовпадение сигнатур**: различные параметры/типы
