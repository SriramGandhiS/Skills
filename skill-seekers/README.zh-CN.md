...
- PASS: **久经考验** — 2,540+ 测试，24+ 框架预设，生产就绪
...
### 文档抓取
- PASS: **llms.txt 支持** - 自动检测并使用 LLM 就绪文档文件（快 10 倍）
- PASS: **通用抓取器** - 适用于任意文档网站
- PASS: **智能分类** - 按主题自动组织内容
- PASS: **代码语言检测** - 识别 Python、JavaScript、C++、GDScript 等
- PASS: **24+ 即用预设** - Godot、React、Vue、Django、FastAPI 等

### PDF 支持
- PASS: **基础 PDF 提取** - 从 PDF 提取文本、代码和图片
- PASS: **扫描件 OCR** - 从扫描文档提取文本
- PASS: **密码保护 PDF** - 处理加密 PDF
- PASS: **表格提取** - 提取复杂表格
- PASS: **并行处理** - 大型 PDF 快 3 倍
- PASS: **智能缓存** - 重复运行快 50%

### 视频提取
- PASS: **YouTube 和本地视频** - 从视频提取字幕、代码和结构化知识
- PASS: **视觉帧分析** - 屏幕 OCR 提取代码编辑器、终端和幻灯片内容
- PASS: **GPU 自动检测** - 自动安装正确的 PyTorch 版本（CUDA/ROCm/MPS/CPU）
- PASS: **AI 增强** - 两阶段增强：清理 OCR + 生成精美 SKILL.md
- PASS: **时间裁剪** - 提取视频的特定片段（`--start-time`、`--end-time`）
- PASS: **播放列表支持** - 批量处理 YouTube 播放列表中的所有视频

### GitHub 仓库分析
- PASS: **深度代码分析** - 支持 Python、JavaScript、TypeScript、Java、C++、Go 的 AST 解析
- PASS: **API 提取** - 函数、类、方法及参数和类型
- PASS: **仓库元数据** - README、文件树、语言统计、星标/分支数
- PASS: **GitHub Issues 和 PR** - 获取带标签和里程碑的开放/已关闭 issues
- PASS: **CHANGELOG 和发布** - 自动提取版本历史
- PASS: **冲突检测** - 对比文档化 API 与实际代码实现
- PASS: **MCP 集成** - 自然语言："抓取 GitHub 仓库 facebook/react"

### 统一多源抓取
- PASS: **合并多个来源** - 在一个技能中混合文档 + GitHub + PDF
- PASS: **冲突检测** - 自动发现文档与代码之间的差异
- PASS: **智能合并** - 基于规则或 AI 驱动的冲突解决
- PASS: **透明报告** - 带 WARNING: 警告的并排对比
- PASS: **文档差距分析** - 识别过时文档和未文档化功能
- PASS: **单一事实来源** - 一个技能同时展示意图（文档）和现实（代码）
- PASS: **向后兼容** - 遗留单源配置继续有效

### 多 LLM 平台支持
- PASS: **12 个 LLM 平台** - Claude AI、Google Gemini、OpenAI ChatGPT、MiniMax AI、通用 Markdown、OpenCode、Kimi、DeepSeek、Qwen、OpenRouter、Together AI、Fireworks AI
- PASS: **通用抓取** - 相同文档适用于所有平台
- PASS: **平台专用打包** - 针对每个 LLM 的优化格式
- PASS: **一键导出** - `--target` 标志选择平台
- PASS: **可选依赖** - 仅安装所需内容
- PASS: **100% 向后兼容** - 现有 Claude 工作流无需更改

| 平台 | 格式 | 上传 | 增强 | API Key | 自定义端点 |
|------|------|------|------|---------|-----------|
| **Claude AI** | ZIP + YAML | PASS: 自动 | PASS: 是 | ANTHROPIC_API_KEY | ANTHROPIC_BASE_URL |
| **Google Gemini** | tar.gz | PASS: 自动 | PASS: 是 | GOOGLE_API_KEY | - |
| **OpenAI ChatGPT** | ZIP + Vector Store | PASS: 自动 | PASS: 是 | OPENAI_API_KEY | - |
| **通用 Markdown** | ZIP | FAIL: 手动 | FAIL: 否 | - | - |
...
### 三流 GitHub 架构
- PASS: **三流分析** - 将 GitHub 仓库拆分为代码流、文档流和洞察流
- PASS: **统一代码库分析器** - 同时适用于 GitHub URL 和本地路径
- PASS: **C3.x 分析深度** - 选择"basic"（1–2 分钟）或"c3x"（20–60 分钟）分析
- PASS: **增强路由生成** - GitHub 元数据、README 快速入门、常见问题
- PASS: **Issue 集成** - 来自 GitHub Issues 的常见问题和解决方案
- PASS: **智能路由关键词** - GitHub 标签权重加倍，提升主题检测效果
...
### 私有配置仓库
- PASS: **基于 Git 的配置源** - 从私有/团队 Git 仓库获取配置
- PASS: **多源管理** - 注册无限数量的 GitHub、GitLab、Bitbucket 仓库
- PASS: **团队协作** - 在 3–5 人团队间共享自定义配置
- PASS: **企业支持** - 扩展到 500+ 开发者
- PASS: **安全认证** - 环境变量 token（GITHUB_TOKEN、GITLAB_TOKEN）

### 代码库分析（C3.x）

**C3.4：配置模式提取（含 AI 增强）**
- PASS: **9 种配置格式** - JSON、YAML、TOML、ENV、INI、Python、JavaScript、Dockerfile、Docker Compose
- PASS: **7 种模式类型** - 数据库、API、日志、缓存、邮件、认证、服务器配置
- PASS: **AI 增强** - 可选双模式 AI 分析（API + LOCAL）
- PASS: **安全分析** - 发现硬编码的密钥和暴露的凭证

**C3.3：AI 增强操作指南**
- PASS: **全面 AI 增强** - 将基础指南转换为专业教程
- PASS: **5 项自动改进** - 步骤说明、故障排除、前提条件、后续步骤、使用场景
- PASS: **双模式支持** - API 模式（Claude API）或 LOCAL 模式（Claude Code CLI）
- PASS: **LOCAL 模式零成本** - 使用您的 Claude Code Max 计划免费增强
...
- PASS: **5 个内置预设** — `default`、`minimal`、`security-focus`、`architecture-comprehensive`、`api-documentation`
- PASS: **用户自定义预设** — 将自定义工作流添加到 `~/.config/skill-seekers/workflows/`
- PASS: **多工作流链式** — 在一条命令中链式使用两个或更多工作流
- PASS: **完整 CLI 管理** — 列出、查看、复制、添加、删除和验证工作流
...
### 性能与规模
- PASS: **异步模式** - 使用 async/await 抓取速度快 2–3 倍（使用 `--async` 标志）
- PASS: **大型文档支持** - 通过智能拆分处理 10K–40K+ 页文档
- PASS: **路由器/Hub 技能** - 智能路由到专用子技能
- PASS: **并行抓取** - 同时处理多个技能
- PASS: **检查点/续传** - 长时间抓取永不丢失进度
- PASS: **缓存系统** - 抓取一次，即时重建

### PASS: 质量保证
- PASS: **全面测试** - 2,540+ 测试，全面覆盖
...
| 上传 | PASS: API | PASS: API | PASS: API | FAIL: 手动 |
| 增强 | PASS: Sonnet 4 | PASS: 2.0 Flash | PASS: GPT-4o | FAIL: 无 |
| 所有技能模式 | PASS: | PASS: | PASS: | PASS: |
