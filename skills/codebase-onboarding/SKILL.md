...
1. Package manifest detection
   â†’ package.json, go.mod, Cargo.toml, pyproject.toml, pom.xml, build.gradle,
     Gemfile, composer.json, mix.exs, pubspec.yaml

2. Framework fingerprinting
   â†’ next.config.*, nuxt.config.*, angular.json, vite.config.*,
     django settings, flask app factory, fastapi main, rails config

3. Entry point identification
   â†’ main.*, index.*, app.*, server.*, cmd/, src/main/

4. Directory structure snapshot
   â†’ Top 2 levels of the directory tree, ignoring node_modules, vendor,
     .git, dist, build, __pycache__, .next

5. Config and tooling detection
   â†’ .eslintrc*, .prettierrc*, tsconfig.json, Makefile, Dockerfile,
     docker-compose*, .github/workflows/, .env.example, CI configs

6. Test structure detection
   â†’ tests/, test/, __tests__/, *_test.go, *.spec.ts, *.test.js,
...
<!-- Example for a React project â€” replace with detected directories -->
```
src/components/  â†’ React UI components
src/api/         â†’ API route handlers
src/lib/         â†’ Shared utilities
src/db/          â†’ Database models and migrations
tests/           â†’ Test suites
...
## Tech Stack
<!-- Example for a Next.js project â€” replace with detected stack -->
...
## Key Entry Points
<!-- Example for a Next.js project â€” replace with detected paths -->
- **API routes**: `src/app/api/` â€” Next.js route handlers
- **UI pages**: `src/app/(dashboard)/` â€” authenticated pages
- **Database**: `prisma/schema.prisma` â€” data model source of truth
- **Config**: `next.config.ts` â€” build and runtime config

## Directory Map
[Top-level directory â†’ purpose mapping]
...
## Common Tasks
<!-- Example for a Node.js project â€” replace with detected commands -->
...
1. **Don't read everything** â€” reconnaissance should use Glob and Grep, not Read on every file. Read selectively only for ambiguous signals.
2. **Verify, don't guess** â€” if a framework is detected from config but the actual code uses something different, trust the code.
3. **Respect existing CLAUDE.md** â€” if one already exists, enhance it rather than replacing it. Call out what's new vs existing.
4. **Stay concise** â€” the onboarding guide should be scannable in 2 minutes. Details belong in the code, not the guide.
5. **Flag unknowns** â€” if a convention can't be confidently detected, say so rather than guessing. "Could not determine test runner" is better than a wrong answer.

## Anti-Patterns to Avoid

- Generating a CLAUDE.md that's longer than 100 lines â€” keep it focused
- Listing every dependency â€” highlight only the ones that shape how you write code
- Describing obvious directory names â€” `src/` doesn't need an explanation
- Copying the README â€” the onboarding guide adds structural insight the README lacks

## Examples

### Example 1: First time in a new repo
**User**: "Onboard me to this codebase"
**Action**: Run full 4-phase workflow â†’ produce Onboarding Guide + Starter CLAUDE.md
