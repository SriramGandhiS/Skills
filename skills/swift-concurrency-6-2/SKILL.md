...
description: Swift 6.2 Approachable Concurrency â€” single-threaded by default, @concurrent for explicit background offloading, isolated conformances for main actor types.
---
...
```swift
// Swift 6.2: OK â€” async stays on MainActor, no data race
@MainActor
final class StickerModel {
    let photoProcessor = PhotoProcessor()
...
## Core Pattern â€” Isolated Conformances
...
// Swift 6.1: ERROR â€” crosses into main actor-isolated code
...
```swift
// OK â€” ImageExporter is also @MainActor
@MainActor
...
// ERROR â€” nonisolated context can't use MainActor conformance
...
## Core Pattern â€” Global and Static Variables

Protect global/static state with MainActor:

```swift
// Swift 6.1: ERROR â€” non-Sendable type may have shared mutable state
...
Swift 6.2 introduces a mode where MainActor is inferred by default â€” no manual annotations needed:
...
## Core Pattern â€” @concurrent for Background Work

When you need actual parallelism, explicitly offload with `@concurrent`:

> **Important:** This example requires Approachable Concurrency build settings â€” SE-0466 (MainActor default isolation) and SE-0461 (NonisolatedNonsendingByDefault). With these enabled, `extractSticker` stays on the caller's actor, making mutable state access safe. **Without these settings, this code has a data race** â€” the compiler will flag it.
...
| Opt-in adoption | Non-breaking migration path â€” enable features incrementally |
...
- **Start on MainActor** â€” write single-threaded code first, optimize later
- **Use `@concurrent` only for CPU-intensive work** â€” image processing, compression, complex computation
...
- Fighting the compiler â€” if it reports a data race, the code has a real concurrency issue
