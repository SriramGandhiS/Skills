...
- **Dev mode** serves source files as native ESM â€” no bundling. Transforms happen on-demand per module request, which is why cold starts are fast and HMR is precise.
- **Build mode** uses Rolldown (v7+) or Rollup (v5â€“v6) to bundle the app for production with tree-shaking, code-splitting, and Oxc-based minification.
...
- **Plugins** share a unified interface across dev and build â€” the same plugin object works for both the dev server's on-demand transforms and the production pipeline.
...
Authoring is rare â€” most needs are covered by existing plugins. When you do need one, start inline in `vite.config.ts` and only extract if reused.

```typescript
// vite.config.ts â€” minimal inline plugin
...
All `import.meta.hot` code is tree-shaken out of production builds â€” no guard removal needed.
...
```typescript
// BAD: passing '' as the third arg loads ALL env vars â€” including server secrets â€”
...
```typescript
build: {
  sourcemap: false,                                  // default â€” keep it this way
}
```
...
```typescript
// vite.config.ts â€” build.rolldownOptions
build: {
  rolldownOptions: {
    output: {
...
```typescript
// BAD â€” importing one util forces Vite to load the whole barrel
import { slash } from '@/utils'

// GOOD â€” direct import, only the one file is loaded
...
`server.warmup.clientFiles` pre-transforms known hot entries before the browser requests them â€” eliminating the cold-load request waterfall on large apps.
...
```typescript
// vite.config.ts â€” Docker/container setup
server: {
...
```typescript
// vite.config.ts â€” monorepo file access
server: {
  fs: {
...
// BAD: Assuming require() works in application source code â€” Vite is ESM-first
const lib = require('some-lib')                // use import instead

// BAD: Splitting every node_module into its own chunk â€” creates hundreds of tiny files
manualChunks(id) {
...
// BAD: Not externalizing peer deps in library mode â€” causes duplicate runtime errors
