...
Patterns for implementing Apple's Liquid Glass â€” a dynamic material that blurs content behind it, reflects color and light from surrounding content, and reacts to touch and pointer interactions. Covers SwiftUI, UIKit, and WidgetKit integration.
...
The `spacing` parameter controls merge distance â€” closer elements blend their glass shapes together.
...
To allow horizontal scroll content to extend under a sidebar or inspector, ensure the `ScrollView` content reaches the leading/trailing edges of the container. The system automatically handles the under-sidebar scrolling behavior when the layout extends to the edges â€” no additional modifier is needed.

## Core Pattern â€” UIKit
...
- **Always use GlassEffectContainer** when applying glass to multiple sibling views â€” it enables morphing and improves rendering performance
...
- **Test across appearances** â€” light mode, dark mode, and accented/tinted modes
- **Ensure accessibility contrast** â€” text on glass must remain readable

## Anti-Patterns to Avoid

- Using multiple standalone `.glassEffect()` views without a GlassEffectContainer
- Nesting too many glass effects â€” degrades performance and visual clarity
- Applying glass to every view â€” reserve for interactive elements, toolbars, and cards
- Forgetting `clipsToBounds = true` in UIKit when using corner radii
- Ignoring accented rendering mode in widgets â€” breaks tinted Home Screen appearance
- Using opaque backgrounds behind glass â€” defeats the translucency effect
