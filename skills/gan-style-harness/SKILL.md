...
> When asked to evaluate their own work, agents are pathological optimists â€” they praise mediocre output and talk themselves out of legitimate issues. But engineering a **separate evaluator** to be ruthlessly strict is far more tractable than teaching a generator to self-critique.
...
```
                    ...
                    â””â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”˜
                           â”‚ Product Spec
                           â”‚ (features, sprints, design direction)
                           â–¼
              â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
              â”‚                        â”‚
              â”‚   GENERATOR-EVALUATOR  â”‚
              â”‚      FEEDBACK LOOP     â”‚
              â”‚                        â”‚
              â”‚  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”          â”‚
              â”‚  â”‚GENERATOR â”‚--build-->â”‚â”€â”€â”
              â”‚  â”‚(Opus 4.6)â”‚          â”‚  â”‚
              â”‚  â””â”€â”€â”€â”€â–²â”€â”€â”€â”€â”€â”˜          â”‚  â”‚
              â”‚       â”‚                â”‚  â”‚ live app
              â”‚    feedback             â”‚  â”‚
...
**Role:** Product manager â€” expands a brief prompt into a full product specification.
...
- Is deliberately **ambitious** â€” conservative planning leads to underwhelming results
...
**Role:** Developer â€” implements features according to the spec.
...
**Role:** QA engineer â€” tests the live running application, not just code.
...
- Scores against four criteria (configurable):
  1. **Design Quality** â€” Does it feel like a coherent whole?
  2. **Originality** â€” Custom decisions vs. template/AI patterns?
  3. **Craft** â€” Typography, spacing, animations, micro-interactions?
  4. **Functionality** â€” Do all features actually work?
- Returns structured feedback with scores and specific issues
- Is engineered to be **ruthlessly strict** â€” never praises mediocre work
...
### Stage 1 â€” Weaker Models (Sonnet-class)
...
### Stage 2 â€” Capable Models (Opus 4.5-class)
...
### Stage 3 â€” Frontier Models (Opus 4.6-class)
...
1. **Evaluator too lenient** â€” If the evaluator passes everything on iteration 1, your rubric is too generous. Tighten scoring criteria and add explicit penalties for common AI patterns.

2. **Generator ignoring feedback** â€” Ensure feedback is passed as a file, not inline. The generator should read `feedback-NNN.md` at the start of each iteration.

3. **Infinite loops** â€” Always set `GAN_MAX_ITERATIONS`. If the generator can't improve past a score plateau after 3 iterations, stop and flag for human review.

4. **Evaluator testing superficially** â€” The evaluator must use Playwright to **interact** with the live app, not just screenshot it. Click buttons, fill forms, test error states.

5. **Evaluator praising its own fixes** â€” Never let the evaluator suggest fixes and then evaluate those fixes. The evaluator only critiques; the generator fixes.

6. **Context exhaustion** â€” For long sessions, use Claude Agent SDK's automatic compaction or reset context between major phases.
...
- [Anthropic: Harness Design for Long-Running Apps](https://www.anthropic.com/engineering/harness-design-long-running-apps) â€” Original paper by Prithvi Rajasekaran
