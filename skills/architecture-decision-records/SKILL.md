...
5. **State consequences** â€” what are the trade-offs? What becomes easier/harder?
6. **Assign a number** â€” scan existing ADRs in `docs/adr/` and increment
7. **Confirm and write** â€” present the draft ADR to the user for review. Only write to `docs/adr/NNNN-decision-title.md` after explicit approval. If the user declines, discard the draft without writing any files.
8. **Update the index** â€” append to `docs/adr/README.md`

### Reading Existing ADRs

When a user asks "why did we choose X?":

1. Check if `docs/adr/` exists â€” if not, respond: "No ADRs found in this project. Would you like to start recording architectural decisions?"
...
â””â”€â”€ adr/
    ...
    â”œâ”€â”€ 0001-use-nextjs.md
    â”œâ”€â”€ 0002-postgres-over-mongo.md
    â”œâ”€â”€ 0003-rest-over-graphql.md
    â””â”€â”€ template.md            â† blank template for manual use
```
...
**Implicit signals** (suggest recording an ADR â€” do not auto-create without user confirmation)
...
### Do
- **Be specific** â€” "Use Prisma ORM" not "use an ORM"
- **Record the why** â€” the rationale matters more than the what
- **Include rejected alternatives** â€” future developers need to know what was considered
- **State consequences honestly** â€” every decision has trade-offs
- **Keep it short** â€” an ADR should be readable in 2 minutes
- **Use present tense** â€” "We use X" not "We will use X"

### Don't
- Record trivial decisions â€” variable naming or formatting choices don't need ADRs
- Write essays â€” if the context section exceeds 10 lines, it's too long
- Omit alternatives â€” "we just picked it" is not a valid rationale
- Backfill without marking it â€” if recording a past decision, note the original date
- Let ADRs go stale â€” superseded decisions should reference their replacement
