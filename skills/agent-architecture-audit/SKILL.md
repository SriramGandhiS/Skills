...
| 7 | Tool execution | Hallucinated execution â€” claims to call but doesn't |
...
- **Target system** â€” what agent application?
- **Entrypoints** â€” how do users interact with it?
- **Model stack** â€” which LLM(s) and providers?
- **Symptoms** â€” what does the user report?
- **Time window** â€” when did it start?
- **Layers to audit** â€” which of the 12 layers apply?

### Phase 2: Evidence Collection

Gather evidence from the codebase:

- **Source code** â€” agent loop, tool router, memory admission, prompt assembly
- **Logs** â€” historical session traces, tool call records
- **Config** â€” prompt templates, tool schemas, provider settings
- **Memory files** â€” SOPs, knowledge bases, session archives
...
- **Symptom** â€” what the user sees
- **Mechanism** â€” how the wrapper causes it
- **Source layer** â€” which of the 12 layers
- **Root cause** â€” the deepest cause
- **Evidence** â€” file:line or log:row reference
...
1. **Code-gate tool requirements** â€” enforce in code, not just prompt text
2. **Remove or narrow hidden repair agents** â€” make fallback explicit with contracts
3. **Reduce context duplication** â€” same info through prompt + history + memory + distillation
4. **Tighten memory admission** â€” user corrections > agent assertions
5. **Tighten distillation triggers** â€” don't compress what shouldn't be compressed
6. **Reduce rendering mutation** â€” pass-through, don't transform
7. **Convert to typed JSON envelopes** â€” structured internal flow, not freeform prose
...
- `agent-introspection-debugging` â€” Debug agent runtime failures (loops, timeouts, state errors)
- `agent-eval` â€” Benchmark agent performance head-to-head
