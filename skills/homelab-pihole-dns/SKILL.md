...
Every device on your network gets ad and malware domain blocking automatically â€” no browser
extension needed.
...
Normal flow (without Pi-hole):
  Device â†’ requests ads.tracker.com â†’ ISP DNS â†’ real IP â†’ ads load

With Pi-hole:
  Device â†’ requests ads.tracker.com â†’ Pi-hole DNS â†’ blocked (returns 0.0.0.0) â†’ no ad

All DNS queries go through Pi-hole first.
Pi-hole checks against blocklists.
Blocked domains return a null response â€” the ad/tracker never loads.
...
## 1. Select network interface (eth0 for wired â€” recommended)
## 2. Select upstream DNS (Cloudflare or leave default â€” can change later)
## 3. Confirm static IP
...
## Method 1: Change DNS in your router DHCP settings (recommended)
  Router admin UI â†’ DHCP Settings â†’ DNS Server
...
## Method 2: Per-device DNS (useful for testing before network-wide rollout)
  Windows: Control Panel â†’ Network Adapter â†’ IPv4 Properties â†’ set DNS manually
  macOS: System Settings â†’ Network â†’ Details â†’ DNS â†’ set manually
  Linux: /etc/resolv.conf or NetworkManager

## Method 3: Pi-hole as DHCP server (replaces router DHCP)
  Pi-hole admin â†’ Settings â†’ DHCP â†’ Enable
  Disable DHCP on your router first â€” two DHCP servers on the same network cause conflicts
...
```
## Pi-hole admin â†’ Adlists â†’ Add new adlist

## Recommended blocklists:
  <https://raw.githubusercontent.com/StevenBlack/hosts/master/hosts>
  # default â€” 200k+ domains
...
## After adding a list:
  Tools â†’ Update Gravity  (downloads and compiles all blocklists)

## If a site is blocked that should not be (false positive):
  Pi-hole admin â†’ Whitelist â†’ Add domain
  Example: api.my-legitimate-service.com

## Check what is being blocked in real time:
  Dashboard â†’ Query Log  (live DNS query stream with block/allow status)
```
...
## Pi-hole admin â†’ Settings â†’ DNS â†’ Custom upstream DNS
...
> The IETF-reserved suffix for local use is `.home.arpa` (RFC 8375) â€” use that to
> follow the standard. Avoid `.local` for Pi-hole DNS records as it conflicts with
> mDNS/Bonjour.

```
## Pi-hole admin â†’ Local DNS â†’ DNS Records
...
## From any device on your network:
  ping nas.home.lan        â†’ 192.168.30.10
  <http://grafana.home.lan>  â†’ your Grafana dashboard

## For subdomains, add a CNAME:
  Pi-hole admin â†’ Local DNS â†’ CNAME Records
  Domain: portainer.home.lan â†’ Target: pi.home.lan
```
...
## New ad and malware domains accumulate â€” stale lists miss them
## GOOD: Schedule weekly gravity update: pihole -g (or enable in Settings â†’ API)
```
...
- Review the Query Log occasionally â€” blocked queries show you what devices are doing
