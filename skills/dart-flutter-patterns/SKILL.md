...
7. **Networking** â€” Dio with interceptors, token refresh with one-time retry guard
8. **Error handling** â€” global capture, `ErrorWidget.builder`, crashlytics wiring
9. **Testing** â€” unit (BLoC test), widget (ProviderScope overrides), fakes over mocks

## Examples

```dart
// Sealed state â€” prevents impossible states
...
```dart
// BAD â€” crashes at runtime if null
final name = user!.name;

// GOOD â€” provide fallback
final name = user?.name ?? 'Unknown';

// GOOD â€” Dart 3 pattern matching (preferred for complex cases)
final display = switch (user) {
  User(:final name, :final email) => '$name <$email>',
  null => 'Guest',
};

// GOOD â€” guard early return
...
```dart
// BAD â€” defers null error to runtime
late String userId;

// GOOD â€” nullable with explicit initialization
String? userId;

// OK â€” use late only when initialization is guaranteed before first access
...
// Exhaustive switch â€” compiler enforces all branches
...
Future<DashboardData> loadDashboard(UserRepository users, OrderRepository orders) async {
  // Run concurrently â€” don't await sequentially
...
// In widget layer â€” declarative, no manual subscription
...
```dart
// CRITICAL â€” always check mounted after any await in StatefulWidget
Future<void> _handleSubmit() async {
  setState(() => _isLoading = true);
  try {
    await authService.login(_email, _password);
    if (!mounted) return; // â† guard before using context
    context.go('/home');
...
```dart
// BAD â€” private method returning widget, prevents optimization
Widget _buildHeader() {
  return Container(
...
// GOOD â€” separate widget class, enables const, element reuse
...
```dart
// BAD â€” entire page rebuilds on every counter change
...
// GOOD â€” isolate the rebuilding part
...
```dart
// Cubit â€” synchronous or simple async state
...
```dart
// Global error capture â€” set up in main()
void main() {
...
- Skill: `flutter-dart-code-review` â€” comprehensive review checklist
- Rules: `rules/dart/` â€” coding style, patterns, security, testing, hooks
