...
7. **SideEffects**: cache served IDs, emit impression events, update counters, log analytics â€” all fire-and-forget
...
- **Single score**: train one model to predict relevance. To change behavior â†’ retrain.
- **Multi-action**: predict `P(action)` for many actions (read, like, share, skip, report), combine with weights at serving time. To change behavior â†’ change weights. No retraining.
...
- **Request-time (online)**: pipeline runs on each request. Latency budget: 100â€“300ms. Default.
...
1. **Do not invent benchmark numbers.** "How much faster?" â†’ "depends on workload, run it yourself."
...
4. **Surface trade-offs.** Multi-action vs single, isolation vs joint, online vs offline â€” never default silently.
...
- Generating pseudocode "for illustration" â€” the scaffold must actually run
...
- 3 runnable example scaffolds, every one green on its test suite:
  - Strapi v5 plugin (TypeScript / Jest â€” 3/3 pass)
  - Zentra-compatible pipeline (Go with generics â€” 3/3 pass)
  - PMAI task prioritizer (Python / FastAPI / pytest â€” 3/3 pass)
