...
description: >-
  ...
  "cuÃ¡ntos tokens", "ahorrar tokens", "responde al 50%", "dame la versiÃ³n
  corta", "quiero controlar cuÃ¡nto usas", or clear variants where the
...
### Step 1 â€” Estimate input tokens
...
### Step 2 â€” Estimate response size by complexity

Classify the prompt, then apply the multiplier range to get the full response window:

| Complexity   | Multiplier range | Example prompts                                      |
|--------------|------------------|------------------------------------------------------|
| Simple       | 3Ã— â€“ 8Ã—          | "What is X?", yes/no, single fact                   |
| Medium       | 8Ã— â€“ 20Ã—         | "How does X work?"                                  |
| Medium-High  | 10Ã— â€“ 25Ã—        | Code request with context                           |
| Complex      | 15Ã— â€“ 40Ã—        | Multi-part analysis, comparisons, architecture      |
| Creative     | 10Ã— â€“ 30Ã—        | Stories, essays, narrative writing                  |

Response window = `input_tokens Ã— mult_min`to`input_tokens Ã— mult_max` (but donâ€™t exceed your modelâ€™s configured output-token limit).

### Step 3 â€” Present depth options
...
Precision: heuristic estimate ~85-90% accuracy (Â±15%).
```

Level token estimates (within the response window):
- 25%  â†’ `min + (max - min) Ã— 0.25`
- 50%  â†’ `min + (max - min) Ã— 0.50`
- 75%  â†’ `min + (max - min) Ã— 0.75`
- 100% â†’ `max`

### Step 4 â€” Respond at the chosen level
...
| 100% Exhaustive  | No restriction      | Everything â€” full analysis, all code, all perspectives | Nothing                                        |

## Shortcuts â€” skip the question
...
This skill uses heuristic estimation â€” no real tokenizer. Accuracy ~85-90%, variance Â±15%. Always show the disclaimer.
...
Standalone skill from [TBA â€” Token Budget Advisor for Claude Code](https://github.com/Xabilimon1/Token-Budget-Advisor-Claude-Code-).
