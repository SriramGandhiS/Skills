...
- PASS: **Works with any deployment**: Not limited to health check failures
- PASS: **Rollback to any version**: Can rollback to any previous deployment, not just the immediate previous one
- PASS: **Uses registry storage**: Images are stored in your registry, making them persistent and accessible
