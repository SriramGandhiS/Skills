...
```yaml
volumes:
  - "../files/my-database:/var/lib/mysql" PASS:
  - "../files/my-configs:/etc/my-app/config" PASS:
```
...
|---------|----------------------|---------------|
| Simple persistence | PASS: Yes | PASS: Yes |
| Direct host access | PASS: Yes | FAIL: No |
| Automated backups | FAIL: No | PASS: Yes |
| Docker-managed | FAIL: No | PASS: Yes |
| Best for config files | PASS: Yes | WARNING: Possible but less common |
| Best for databases | WARNING: Possible | PASS: Recommended |
