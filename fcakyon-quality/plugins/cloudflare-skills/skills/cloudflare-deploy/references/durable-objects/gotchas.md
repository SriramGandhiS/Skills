...
```typescript
// FAIL: Wrong - lost on hibernation
private userCount = 0;
async webSocketMessage(ws: WebSocket, msg: string) {
  this.userCount++;  // Lost!
}

// PASS: Right - persisted
async webSocketMessage(ws: WebSocket, msg: string) {
...
```typescript
// FAIL: Wrong - lost on eviction
setTimeout(() => this.cleanup(), 3600000);

// PASS: Right - survives eviction
...
```typescript
// FAIL: Wrong - expensive on every wake
constructor(ctx: DurableObjectState, env: Env) {
  super(ctx, env);
  this.heavyData = this.loadExpensiveData();  // Slow!
}

// PASS: Right - lazy load
...
```typescript
// FAIL: Wrong - race condition
async incrementCounter() {
  const count = await this.ctx.storage.get("count") || 0;
  // WARNING: Another request could execute here during await
  await this.ctx.storage.put("count", count + 1);
}

// PASS: Right - atomic operation
async incrementCounter() {
