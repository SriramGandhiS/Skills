...
1. PASS: **Catalog enabled:** `npx wrangler r2 bucket catalog status <bucket>`
2. PASS: **Token permissions:** Both R2 Data Catalog + R2 Storage in dashboard
3. PASS: **Connection test:** `catalog.list_namespaces()` succeeds
4. PASS: **URI format:** HTTPS, includes `/iceberg/`, correct bucket name
5. PASS: **Warehouse name:** Matches bucket name exactly
6. PASS: **Namespace exists:** Create before `create_table()`
7. PASS: **Enable debug logging:** `logging.basicConfig(level=logging.DEBUG)`
8. PASS: **PyIceberg version:** `pip install --upgrade pyiceberg` (≥0.5.0)
9. PASS: **File health:** Compact if >1000 files or avg <10MB
10. PASS: **Snapshot count:** Expire if >100 snapshots
