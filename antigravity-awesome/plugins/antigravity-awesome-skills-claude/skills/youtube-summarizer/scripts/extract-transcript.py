...
def extract_transcript(video_id, language='en'):
    ...
    try:
        ...
        print(f"FAIL: Transcripts are disabled for video {video_id}", file=sys.stderr)
        sys.exit(1)
    except NoTranscriptFound:
        print(f"FAIL: No transcript found for video {video_id}", file=sys.stderr)
        sys.exit(1)
...
def list_available_transcripts(video_id):
    ...
    try:
        ...
        print(f"PASS: Available transcripts for {video_id}:")
        ...
        print(f"FAIL: Error listing transcripts: {e}", file=sys.stderr)
        return False
