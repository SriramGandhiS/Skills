...
if python3 -c "import faster_whisper" 2>/dev/null; then
    ...
    echo "PASS: Faster-Whisper detected (optimized)"
# Fallback to original Whisper
elif python3 -c "import whisper" 2>/dev/null; then
    TRANSCRIBER="whisper"
    echo "PASS: OpenAI Whisper detected"
else
    TRANSCRIBER="none"
    echo "WARNING:  No transcription tool found"
fi

# Check for ffmpeg (audio format conversion)
if command -v ffmpeg &>/dev/null; then
    echo "PASS: ffmpeg available (format conversion enabled)"
else
...
if [[ ! "$AUTO_INSTALL" =~ ^[Nn] ]]; then
    ...
    if [[ -f "$SKILL_DIR/scripts/install-requirements.sh" ]]; then
        ...
        echo "FAIL: Installation script not found"
        echo ""
    ...
    if python3 -c "import faster_whisper" 2>/dev/null || python3 -c "import whisper" 2>/dev/null; then
        echo "PASS: Installation successful! Proceeding with transcription..."
    else
...
if [[ ! -f "$AUDIO_FILE" ]]; then
    echo "FAIL: File not found: $AUDIO_FILE"
    exit 1
fi
```
...
if [[ $SIZE_MB -gt 25 ]]; then
    echo "WARNING:  Large file ($FILE_SIZE) - processing may take several minutes"
fi
```
...
if [[ ! " ${SUPPORTED_FORMATS[@]} " =~ " ${EXTENSION,,} " ]]; then
    echo "WARNING:  Unsupported format: $EXTENSION"
    if command -v ffmpeg &>/dev/null; then
        echo " Converting to WAV..."
        ffmpeg -i "$AUDIO_FILE" -ar 16000 "${AUDIO_FILE%.*}.wav" -y
        AUDIO_FILE="${AUDIO_FILE%.*}.wav"
    else
        echo "FAIL: Install ffmpeg to convert formats: brew install ffmpeg"
        exit 1
    fi
fi
```
...
echo "PASS: Transcript salvo: $TRANSCRIPT_FILE"

if [[ -n "$ATA_CONTENT" ]]; then
    echo "$ATA_CONTENT" > "$ATA_FILE"
    echo "PASS: Ata salva: $ATA_FILE"
fi
```
...
PASS: Concluído! Tempo total: 3m 45s
```
...
```bash
echo ""
echo "PASS: Transcription Complete!"
echo ""
echo " Results:"
...
```bash
PASS: Faster-Whisper detected (optimized)
PASS: ffmpeg available (format conversion enabled)
...
PASS: Language detected: Portuguese (pt-BR)
 Speakers identified: 4
 Generating Markdown output...

PASS: Transcription Complete!
...
[1/5] team-standup.mp3 PASS: (2m 34s)
[2/5] client-call.mp3 PASS: (15m 12s)
[3/5] brainstorm-session.mp3 PASS: (8m 47s)
[4/5] product-demo.mp3 PASS: (22m 03s)
[5/5] retrospective.mp3 PASS: (11m 28s)

PASS: Batch Complete!
 Generated 5 Markdown reports
  Total processing time: 6m 15s
```
...
PASS: Faster-Whisper detected (optimized)
 ...
 Size: 87.2 MB
  Duration: 02:15:47
WARNING:  Large file (87.2 MB) - processing may take several minutes
