...
try:
    ...
    print("WARNING:  Installing rich for better UI...")
...
try:
    ...
    print("WARNING:  Installing tqdm for progress bars...")
...
try:
    ...
    try:
        ...
        print("FAIL: Nenhum engine de transcrição encontrado!")
...
def invoke_prompt_engineer(raw_prompt, timeout=90):
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            console.print("[yellow]WARNING:  prompt-engineer não respondeu, usando template padrão[/yellow]")
            return DEFAULT_MEETING_PROMPT

    except subprocess.TimeoutExpired:
        console.print(f"[red]WARNING:  Timeout após {timeout}s, usando template padrão[/red]")
        return DEFAULT_MEETING_PROMPT
    except Exception as e:
        console.print(f"[red]WARNING:  Erro ao invocar prompt-engineer: {e}[/red]")
        return DEFAULT_MEETING_PROMPT
...
def handle_prompt_workflow(user_prompt, transcript):
    ...
    if user_prompt:
        ...
        if prompt_engineer_available:
            ...
            console.print("[yellow]WARNING:  prompt-engineer skill não disponível[/yellow]")
            console.print("[dim]PASS: Usando seu prompt original[/dim]")
            return user_prompt

    # ========== CENÁRIO B: SEM PROMPT - AUTO-GERAÇÃO ==========
    else:
        console.print("\n[yellow]WARNING:  Nenhum prompt fornecido.[/yellow]")

        if not prompt_engineer_available:
            console.print("[yellow]WARNING:  prompt-engineer skill não encontrado[/yellow]")
        ...
        if generate == "n":
            console.print("[dim]PASS: Ok, gerando apenas transcript.md (sem ata)[/dim]")
...
"""
        ...
        console.print("\n[green]PASS: Prompt gerado:[/green]")
...
def process_with_llm(transcript, prompt, cli_tool='claude', timeout=300):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            console.print(f"[red]FAIL: Erro ao processar com {cli_tool}[/red]")
            console.print(f"[dim]{result.stderr[:200]}[/dim]")
            return None

    except subprocess.TimeoutExpired:
        console.print(f"[red]FAIL: Timeout após {timeout}s[/red]")
        return None
    except Exception as e:
        console.print(f"[red]FAIL: Erro: {e}[/red]")
        return None
...
def transcribe_audio(audio_file, model="base"):
    ...
    """
    console.print(f"\n[cyan]  Transcrevendo áudio com {TRANSCRIBER}...[/cyan]")
    ...
    try:
        ...
        console.print(f"[green]PASS: Transcrição completa! Idioma: {data['language'].upper()}[/green]")
        console.print(f"[dim]   {len(data['segments'])} segmentos processados[/dim]")

        return data

    except Exception as e:
        console.print(f"[red]FAIL: Erro na transcrição: {e}[/red]")
        sys.exit(1)
...
def save_outputs(transcript_text, ata_text, audio_file, output_dir="."):
    ...
    console.print(f"[green]PASS: Transcript salvo:[/green] {transcript_filename}")
    ...
    if ata_text:
        ...
        console.print(f"[green]PASS: Ata salva:[/green] {ata_filename}")
...
def main():
    ...
    if not os.path.exists(args.audio_file):
        console.print(f"[red]FAIL: Arquivo não encontrado: {args.audio_file}[/red]")
        sys.exit(1)
    ...
    if not cli_tool:
        console.print("\n[yellow]WARNING:  Nenhuma CLI de IA detectada (Claude ou GitHub Copilot)[/yellow]")
    ...
    console.print(f"\n[green]PASS: CLI detectada: {cli_tool}[/green]")
    ...
    if ata_text:
        console.print("[green]PASS: Ata gerada com sucesso![/green]")
    else:
        console.print("[yellow]WARNING:  Falha ao gerar ata, salvando apenas transcript[/yellow]")

    # Step 5: Salvar arquivos
    console.print("\n[cyan] Salvando arquivos...[/cyan]")
    save_outputs(transcript_text, ata_text, args.audio_file, args.output_dir)

    console.print("\n[bold green]PASS: Concluído![/bold green]")
