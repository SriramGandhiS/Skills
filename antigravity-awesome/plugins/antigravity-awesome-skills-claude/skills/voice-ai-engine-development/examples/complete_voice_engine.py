    ...
    async def _process_transcriptions(self):
        ...
        while True:
            ...
            if not self.is_human_speaking:
                logger.info("WARNING: [INTERRUPT] User interrupted bot")
...
async def conversation_endpoint(websocket: WebSocket):
    ...
    logger.info("PASS: [WEBSOCKET] Client connected")
    ...
    try:
        ...
        logger.info("FAIL: [WEBSOCKET] Client disconnected")
    except Exception as e:
        logger.error(f"FAIL: [WEBSOCKET] Error: {e}", exc_info=True)
    finally:
