    ...
    for root, dirs, files in os.walk(skills_dir):
        ...
        if "SKILL.md" in files:
            ...
            if os.path.islink(skill_path):
                print(f"WARNING: Skipping symlinked skill file: {skill_path}")
                continue
            ...
            try:
                ...
                print(f"WARNING: {skill_path}: YAML error - {e}")
                continue

            if not isinstance(metadata, dict):
                print(f"WARNING: {skill_path}: Frontmatter must be a mapping, skipping")
                continue
