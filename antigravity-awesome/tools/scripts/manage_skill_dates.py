...
def update_skill_frontmatter(skill_path, metadata):
    ...
    try:
        ...
        if old_metadata is None:
            print(f"FAIL: {skill_path}: Could not parse frontmatter")
            return False
        ...
        if fm_end == -1:
            print(f"FAIL: {skill_path}: Could not locate frontmatter boundary")
            return False
        ...
        print(f"FAIL: Error updating {skill_path}: {str(e)}")
        return False
    ...
    for root, dirs, files in os.walk(skills_dir):
        ...
        if "SKILL.md" in files:
            ...
            try:
                ...
                print(f"WARNING:  Error reading {skill_path}: {str(e)}", file=sys.stderr)
...
def add_missing_dates(date_str=None):
    ...
    if not re.match(r'^\d{4}-\d{2}-\d{2}$', date_str):
        print(f"FAIL: Invalid date format: {date_str}. Use YYYY-MM-DD.")
        return False
    ...
    for root, dirs, files in os.walk(skills_dir):
        ...
        if "SKILL.md" in files:
            ...
            try:
                ...
                metadata, _ = parse_frontmatter(content)
                if metadata is None:
                    print(f"WARNING:  {skill_name}: Could not parse frontmatter, skipping")
                    continue

                if 'date_added' not in metadata:
                    if update_skill_frontmatter(skill_path, {'date_added': date_str}):
                        print(f"PASS: {skill_name}: Added date_added: {date_str}")
                        updated_count += 1
                    else:
                        print(f"FAIL: {skill_name}: Failed to update")
                else:
                    skipped_count += 1
            except Exception as e:
                print(f"FAIL: Error processing {skill_name}: {str(e)}")
...
def add_all_dates(date_str=None):
    ...
    for root, dirs, files in os.walk(skills_dir):
        ...
        if "SKILL.md" in files:
            ...
            try:
                if update_skill_frontmatter(skill_path, {'date_added': date_str}):
                    print(f"PASS: {skill_name}: Set date_added: {date_str}")
                    updated_count += 1
                else:
                    print(f"FAIL: {skill_name}: Failed to update")
            except Exception as e:
...
def update_skill_date(skill_name, date_str):
    ...
    if not os.path.exists(skill_path):
        print(f"FAIL: Skill not found: {skill_name}")
        return False

    if update_skill_frontmatter(skill_path, {'date_added': date_str}):
        print(f"PASS: {skill_name}: Updated date_added to {date_str}")
        return True
    else:
        print(f"FAIL: {skill_name}: Failed to update")
        return False
