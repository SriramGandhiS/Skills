...
def main():
    ...
    if len(sys.argv) < 2:
        print("FAIL: 用法錯誤。請提供當前專案的日記絕對路徑。")
    ...
    if not proj_diary_path.exists():
        print(f"WARNING: 找不到專案日記: {proj_diary_path}")
        sys.exit(1)
    ...
    if global_diary_path.exists():
        print("WARNING: 警告：此全域日記已存在，代表今天可能有其他專案寫過進度了！")
        print("WARNING: 鐵律：請務必保留下方既有的內容，只能「追加或融合」新的專案進度，絕對不可粗暴覆寫抹除前人的紀錄！")
        print("-" * 60)
        try:
    ...
    print("\n" + "=" * 60)
    print("PASS: 素材提供完畢。請 IDE Agent 執行融合，並寫入/更新至全域日記檔案。")
