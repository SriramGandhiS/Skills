...
> WARNING: **Ironclad Rule 1**: The local project diary in Step 1 **must NEVER be polluted with global data**. It only records the current project.
> WARNING: **Ironclad Rule 2**: When using tools like `run_command` to execute predefined python scripts (Step 2, Step 4), you must set them to `SafeToAutoRun: true` to avoid getting stuck waiting for user authorization.
> WARNING: **Ironclad Rule 3**: **Never guess the project name**. The AI must confirm the current directory name as the project identifier using a terminal command BEFORE taking any action in Step 1.
...
- **Isolation and Naming Rules (Ironclad Rules)**:
  ...
  - PASS: **Pure Content**: Only record content exclusive to the current project. Do not mix in other projects.
...
1. PASS: Project local diary generated (no pollution).
2. PASS: `fetch_diaries.py` called with absolute path and successfully printed materials.
3. PASS: AI executed high-quality rewrite and precisely wrote to global diary (appended successfully if file existed).
4. PASS: `--sync-only` successfully pushed to Notion + Obsidian.
5. PASS: Experience extraction presented to the user and authorized.
...
2. **Concise Deduplication**: When writing the global diary in Step 3, the AI must condense the " Execution Details" from the local diary. The global diary focuses only on "General Direction and Output Results."
...
## Project Tracking
(WARNING: AI Rule: If file exists, find the corresponding project title and append; NEVER overwrite, keep it clean.)
...
(WARNING: Dedicated to Experience Extraction)
