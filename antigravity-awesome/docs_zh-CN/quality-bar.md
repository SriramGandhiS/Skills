...
**必须满足：**
- PASS: **准确性**：技术信息正确，无错误概念
- PASS: **完整性**：涵盖主要使用场景，步骤完整
- PASS: **清晰性**：语言简洁明了，逻辑清楚
- PASS: **实用性**：解决实际问题，有明确价值
...
**前置内容要求：**
- PASS: `name`：小写+连字符，与目录名匹配
- PASS: `description`：200字符以内，简洁明确
- PASS: `category`：正确的分类标识
- PASS: `risk`：合适的风险级别
- PASS: `date_added`：YYYY-MM-DD格式
- PASS: 必需字段完整，无拼写错误

**内容结构要求：**
- PASS: 标准章节结构（概述、使用场景、工作原理等）
- PASS: 代码块语法正确
- PASS: 链接有效且格式正确
- PASS: 技能引用使用`@skill-name`格式
